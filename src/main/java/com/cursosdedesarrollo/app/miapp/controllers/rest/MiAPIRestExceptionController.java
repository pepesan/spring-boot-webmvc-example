...
@RestController
@RequestMapping("/api/v1/alumnoexcepciones")
public class MiAPIRestExceptionController {
    @Autowired
    ...
    @GetMapping("/{id}")
    public ResponseEntity<AlumnoDevuelto> showById(
        ...
        if (alumnoOptional.isPresent()){
            ...
            throw new AlumnoNotFoundException("Alumno not Found");
        }
        return new ResponseEntity<>(alumnoDevuelto,HttpStatus.OK);
    }
    @PutMapping("/{id}")
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<AlumnoDevuelto> deleteById(@PathVariable("id") Long id) throws AlumnoNotFoundException {
        Optional<Alumno> alumnoOptional = alumnoRepository.findById(id);
        AlumnoDevuelto alumnoDevuelto = new AlumnoDevuelto();
        if (alumnoOptional.isPresent()){
